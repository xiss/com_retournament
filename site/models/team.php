	...
	public function getTeam()
	{
		...
		$query = "
					SELECT jos_rt_teams.name AS name,
                        (SELECT ROUND(AVG(rating)) FROM jos_rt_participants WHERE team_id = $id) AS avg_rating,
                        (SELECT COUNT(jos_rt_participants.id) FROM jos_rt_participants WHERE team_id = $id) AS qt_teammates,
                        (SELECT SUM(warnings) FROM jos_rt_participants WHERE team_id = $id) AS sum_warnings,
                        (SELECT SUM(wins) FROM jos_rt_participants WHERE team_id = $id) AS sum_wins,
                        (SELECT SUM(loses) FROM jos_rt_participants WHERE team_id = $id) AS sum_loses,
                        (SELECT SUM(draws) FROM jos_rt_participants WHERE team_id = $id) AS sum_draws
