		...
		$query = "
					SELECT 
                        ...
                        `type`,
                         ...
                         place_2_id,
                         place_3_id,
                         (SELECT IF(ISNULL(nick), CONCAT_WS(' ', surname, name), nick) FROM jos_rt_participants WHERE id = place_1_id) AS place_1_name,
                         (SELECT IF(ISNULL(nick), CONCAT_WS(' ', surname, name), nick) FROM jos_rt_participants WHERE id = place_2_id) AS place_2_name,
                         (SELECT IF(ISNULL(nick), CONCAT_WS(' ', surname, name), nick) FROM jos_rt_participants WHERE id = place_3_id) AS place_3_name,
						 qt_participants,
                    -- Количество боев в турнире
                        (SELECT COUNT(id) FROM `jos_rt_fights` WHERE `tournament_id` = $id) AS qt_fights,
                    -- Количество ничьих
                        (SELECT COUNT(id) FROM `jos_rt_fights` WHERE `tournament_id` = $id AND (inf_hits_1 <> 3 AND inf_hits_2 <> 3) ) AS qt_draws,
                    -- Количество предупреждений
                        (SELECT (SUM(warnings_1)+ SUM(warnings_2)) FROM `jos_rt_fights` WHERE `tournament_id` = $id) AS qt_warnings
                    FROM `jos_rt_tournaments` WHERE id = $id";
		$db->setQuery($query);
